...
function designation_optionlist(){
   ...
   $('.timesheetSearch').on('change' , function(){
	...
	$('#saveSearch').on('click' , function(){
		...
		$('#brand_Linkidin').on('click' , function(){
			if($('#brand_Linkidin').is(':checked'))
			{
				//alert(1);
				window.open(ajax_url+'linkedinDetails',null,
"height=200,width=450,status=yes,toolbar=no,menubar=no,location=no");

				//$(location).attr('href', ajax_url+'linkedinDetails')
			}
