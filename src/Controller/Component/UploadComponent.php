...
class UploadComponent extends Component
{
	...
	function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
		...
		if (!is_null($allowed)) { $this->_allowed = $allowed; } else { $this->_allowed = array('jpg','jpeg','gif','png','GIF','JPEG','JPG','PNG'); }
	...
	function error ($message) {
		throw new \Cake\Network\Exception\BadRequestException($message);
	}	
	...
	function image ($file, $type, $size, $output = NULL, $quality = NULL) {
		...
		// -- check output
		if ($output != 'jpg' && $output != 'png' && $output != 'gif'&& $output != 'jpeg') {
		...
		switch ($type) {
			...
			case 'resize':
				...
				$this->removeTransparency($dstImg);
		...
		switch ($output) {
			...
			case 'jpeg':
				...
				break;
			case 'png':
				$write = imagepng($dstImg, $this->_name, ($quality/10)-1);
				break;
			case 'gif':
	...
	function removeTransparency($dstImg)
	{
